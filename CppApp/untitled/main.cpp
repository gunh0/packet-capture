...
int main(int argc, char *argv[])
{
    ...
    while (true)
    {
        ...
        ethernet_h *Eth = (ethernet_h *)packet;
        printf(" Source");
        ...
        if (ntohs(Eth->ether_type) == 0x0800)
        {
            ...
            ip_header *IP = (ip_header *)(packet + sizeof(ethernet_h));

            if (IP->version < 4 || IP->version > 9)
                break;
            ...
            {
                tcp_header *P_TCP = (tcp_header *)(packet + sizeof(ethernet_h) + IP->header_len * 4);

                printf(" Source Port Number : %d\n", ntohs(P_TCP->source_port));
                printf(" Destination Port Number : %d\n", ntohs(P_TCP->dest_port));

                int TCP_Data_Len = header->caplen - (sizeof(ethernet_h) + IP->header_len * 4 + sizeof(tcp_header));
                printf(" TCP Data Length : %d\n", TCP_Data_Len);
                if (TCP_Data_Len > 0)
                {
                    printf(" Print Data : ");
                    print_pData(TCP_Data_Len, packet + sizeof(ethernet_h) + IP->header_len * 4 + sizeof(tcp_header));
                }
            }
        }
        pcap_close(handle);
